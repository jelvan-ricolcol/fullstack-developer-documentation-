...
## 1. Overview
- **What it is:** A senior-level implementation guide for Cloudflare Durable Object Realtime within a production full-stack application.
- **Why it is used:** It standardizes architecture, setup, coding, deployment, operations, and maintenance decisions.
- **When to use it:** Use this page when designing, reviewing, building, or operating Cloudflare Durable Object Realtime work.
...
## 2. Architecture
- **System design:** Place Cloudflare Durable Object Realtime behind clear interfaces with ownership, versioning, observability, and rollback plans.
...
- **Project structure:** `src/features/cloudflare-durable-object-realtime/`, `src/shared/`, `tests/`, `migrations/`, `.github/workflows/`.
...
CREATE TABLE users (id TEXT PRIMARY KEY, email TEXT UNIQUE NOT NULL, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP);
CREATE TABLE messages (id TEXT PRIMARY KEY, user_id TEXT NOT NULL REFERENCES users(id), body TEXT NOT NULL, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP);
CREATE INDEX idx_messages_user_created ON messages(user_id, created_at DESC);
```

```ts
// src/index.ts
export interface Env { DB: D1Database; CACHE: KVNamespace; FILES: R2Bucket }
export default { async fetch(request: Request, env: Env) {
  ...
  if (url.pathname === "/users" && request.method === "POST") {
    ...
    await env.CACHE.put(`user:${id}`, JSON.stringify({ id, email }), { expirationTtl: 300 });
