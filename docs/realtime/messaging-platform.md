...
## 1. Overview
- **What it is:** A senior-level implementation guide for Messaging Platform within a production full-stack application.
- **Why it is used:** It standardizes architecture, setup, coding, deployment, operations, and maintenance decisions.
- **When to use it:** Use this page when designing, reviewing, building, or operating Messaging Platform work.
...
## 2. Architecture
- **System design:** Place Messaging Platform behind clear interfaces with ownership, versioning, observability, and rollback plans.
...
- **Project structure:** `src/features/messaging-platform/`, `src/shared/`, `tests/`, `migrations/`, `.github/workflows/`.
...
```ts
// Durable Object room sketch
export class ChatRoom { sessions = new Set<WebSocket>();
  fetch(request: Request) { const pair = new WebSocketPair(); const [client, server] = Object.values(pair); server.accept(); this.sessions.add(server); server.addEventListener("message", e => this.broadcast(String(e.data))); return new Response(null, { status: 101, webSocket: client }); }
  broadcast(message: string) { for (const ws of this.sessions) ws.send(message); }
}
```

```js
const ws = new WebSocket("wss://example.com/room/general");
ws.onmessage = event => console.log(JSON.parse(event.data));
ws.send(JSON.stringify({ type: "message", body: "hello" }));
```
