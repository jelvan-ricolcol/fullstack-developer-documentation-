...
## 1. Overview
- **What it is:** A senior-level implementation guide for Optimization within a production full-stack application.
- **Why it is used:** It standardizes architecture, setup, coding, deployment, operations, and maintenance decisions.
- **When to use it:** Use this page when designing, reviewing, building, or operating Optimization work.
...
## 2. Architecture
- **System design:** Place Optimization behind clear interfaces with ownership, versioning, observability, and rollback plans.
...
- **Project structure:** `src/features/optimization/`, `src/shared/`, `tests/`, `migrations/`, `.github/workflows/`.
...
```sql
CREATE TABLE accounts (id UUID PRIMARY KEY, name TEXT NOT NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT now());
CREATE TABLE invoices (id UUID PRIMARY KEY, account_id UUID NOT NULL REFERENCES accounts(id), status TEXT NOT NULL CHECK (status IN ('draft','open','paid','void')), total_cents INTEGER NOT NULL CHECK (total_cents >= 0), created_at TIMESTAMPTZ NOT NULL DEFAULT now());
