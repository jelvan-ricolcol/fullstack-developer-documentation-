...
## 1. Overview
- **What it is:** A senior-level implementation guide for Integrations within a production full-stack application.
- **Why it is used:** It standardizes architecture, setup, coding, deployment, operations, and maintenance decisions.
- **When to use it:** Use this page when designing, reviewing, building, or operating Integrations work.
...
## 2. Architecture
- **System design:** Place Integrations behind clear interfaces with ownership, versioning, observability, and rollback plans.
...
- **Project structure:** `src/features/integrations/`, `src/shared/`, `tests/`, `migrations/`, `.github/workflows/`.
...
export async function summarize(input: string) {
  const response = await client.responses.create({ model: "gpt-4.1-mini", input: `Summarize for support staff: ${input}` });
