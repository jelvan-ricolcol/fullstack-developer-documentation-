...
## 1. Overview
- **What it is:** A senior-level implementation guide for Authentication within a production full-stack application.
- **Why it is used:** It standardizes architecture, setup, coding, deployment, operations, and maintenance decisions.
- **When to use it:** Use this page when designing, reviewing, building, or operating Authentication work.
...
## 2. Architecture
- **System design:** Place Authentication behind clear interfaces with ownership, versioning, observability, and rollback plans.
...
- **Project structure:** `src/features/authentication/`, `src/shared/`, `tests/`, `migrations/`, `.github/workflows/`.
...
export function issueAccessToken(user: { id: string; roles: string[] }) {
  return jwt.sign({ sub: user.id, roles: user.roles }, process.env.JWT_PRIVATE_KEY!, { algorithm: "RS256", expiresIn: "15m", issuer: "app" });
}
export function requireRole(role: string) { return (req, res, next) => req.user?.roles?.includes(role) ? next() : res.status(403).json({ error: "forbidden" }); }
```

```sql
CREATE TABLE refresh_tokens (id TEXT PRIMARY KEY, user_id TEXT NOT NULL, token_hash TEXT NOT NULL, expires_at TIMESTAMPTZ NOT NULL, revoked_at TIMESTAMPTZ);
